...
function GameDishes() {
  ...
  this.level = 3;
  this.dishSearchWord = '';
  this.totallyFirstWords = [];
  this.totallyFullWords = [];
  this.selectedDishes = [];
  this.pairsClicked = 0;
  this.correctPairs = 0;
}
...
GameDishes.prototype.getRandomDish = function() {
  ...
  this.dishFullWord = this.dishes[random].name.toLowerCase();

  console.log('La palabra de búsqueda es: ' + this.dishFullWord);
  this.totallyFullWords.push(this.dishFullWord);
  this.totallyFirstWords.push(this.dishFullWord.split(' ')[0]);
...
GameDishes.prototype.searchEqual = function() {
  var dishFound = false;
  for (var i = 0; i < this.dishes.length && !dishFound; i++) {
    var myDish = this.dishes[i].name.toLowerCase();
    var firstWord = myDish.split(' ')[0];
    if ( (myIndex = this.totallyFirstWords.indexOf(firstWord) ) !== -1 ){
      if(myDish != this.totallyFullWords[myIndex]){
        this.dishesBoard.push(this.dishes[i]);
        this.totallyFirstWords.pop();
        this.totallyFullWords.pop();
        dishFound = true;
      }
    }
  }
  if (!dishFound) {
    console.log("no hay mas platos coincidentes, hacemos un nuevo random");
